"""
core.py — shared logic for beginner + intermediate Streamlit apps
"""
...
# ── CODE EXECUTION (server-side, 10s timeout) ────────────────────
...
def check_attempts(email, level):
    """Returns (next_attempt_num, is_blocked)."""
    cutoff = (datetime.now(timezone.utc) - timedelta(days=3)).isoformat()
    res = _sb().table("attempts")\
        .select("id")\
...
| **Pass Mark** | 80% |
| **Max Attempts** | 3 &nbsp;*(in any 3-day window)* |
...
""")
    ...
    if st.button("▶  Start Test", type="primary"):
        ...
        if blocked:
            st.error("⛔  You have used all 3 attempts in the last 3 days.")
            return
...
def _test_phase(tests_map, time_limit):
    ...
    with col2:
        color = "red" if tl < 300 else "orange" if tl < (time_limit * 0.25) else "green"
        st.markdown(
            f"<div style='text-align:right'>"
            f"<span style='font-size:1.4rem;font-weight:700;color:{color}'>⏱ {fmt_time(tl)}</span>"
            f"</div>",
    ...
    st.markdown(
        f"**Q{q['id']}** &nbsp;&nbsp;"
        f"<span style='background:#e8f0fe;padding:2px 8px;border-radius:4px;font-size:0.8rem'>{badge[q['type']]}</span>"
        f"&nbsp;&nbsp;"
        f"<span style='background:#fef9c3;padding:2px 8px;border-radius:4px;font-size:0.8rem'>{q['marks']} mark{'s' if q['marks']>1 else ''}</span>",
        unsafe_allow_html=True
    )
    st.markdown("")
    ...
    if q["type"] == "code":
        # show question text + preloaded context in code block
        st.info(q["text"])
    else:
        # MCQ / fill — show in code block if it contains code, else plain
        if "\n" in q["text"]:
            parts = q["text"].split("\n\n", 1)
            if len(parts) == 2:
                st.write(parts[0])
                ...
                st.code(q["text"], language="python")
        else:
            st.write(q["text"])
    ...
    if q["type"] == "mcq":
        ...
        if st.button("Submit →", type="primary", key=f"sub_{q_idx}"):
            if val is None:
                ...
                _record(q, val, q_list, time_limit)
        ...
        code_val = st.text_area(
            "Code editor:", height=160, key=f"inp_{q_idx}",
            placeholder="# write your code here"
        )
        c1, c2 = st.columns([1, 3])
        with c1:
            if st.button("▶ Run", key=f"run_{q_idx}"):
                out = run_code(q["preload"], code_val or "")
                st.session_state.run_out = out
        with c2:
            if st.button("Submit →", type="primary", key=f"sub_{q_idx}"):
                if not (code_val or "").strip():
                    st.warning("Write some code first.")
                else:
                    _record(q, code_val, q_list, time_limit)
...
def _force_submit(time_limit):
    """Auto-submit when timer hits zero."""
    # fill remaining questions as unanswered
    q_list = st.session_state.get("_qlist_cache", [])
    _do_submit(time_limit)
...
def _score_phase(title):
    ...
    if not valid:
        st.error("⚠️  Submission flagged INVALID — time limit was exceeded.")
    elif passed:
        st.success("Congratulations! You passed.")
    else:
        st.warning(f"You needed 80% to pass. Your score: {pct}%")

    st.divider()
    st.subheader("Question Breakdown")
    for r in st.session_state.results:
        icon = "✅" if r["ok"] else "❌"
        st.write(f"{icon} &nbsp; Q{r['id']} — {r['got']} / {r['max']} marks")
